...
class GpsPingFilter:
    ...
    def load_daily_feed(self, source='s3', start_dt=None, end_dt=None):
        ...
        #total_cnt_accumulator = self.spark.sparkContext.accumulator(0)
        #self.df.foreach(lambda row: total_cnt_accumulator.add(1))   
        tot_cnt = self.df.count()
        #tot_cnt = total_cnt_accumulator.value
        print("\n{:,} pings loaded for analysis".format(tot_cnt))
    ...
    def junk_filter(self, offset=240, should_count = True):
        ...
        if should_count is True:                        
            print("init_cnt - num partitions: {num_partitions}".format(num_partitions=init_cnt_df.rdd.getNumPartitions()))
